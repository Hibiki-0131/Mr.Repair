...
public class TerrainState
{
    ...
    public TerrainState(int[,,] csv, bool[,,] solid, float voxelSize, int yOffset)
    {
        csvGrid = csv;
        SolidGrid = solid;
    ...
    public bool TryFillAt(Vector3 localPos, out Vector3 snappedPos)
    {
        ...
        if (SolidGrid[x, y, z]) return false;

        int belowY = y - 1;
        if (belowY < 0) return false;
        if (csvGrid[x, belowY, z] != 4) return false;
